...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT s.id as solution_id, s.price, s.created_at, s.evaluated_at,
    ...
    $stmt = $pdo->prepare("
        SELECT pm.*, s.id as solution_id, p.title as problem_title,
    ...
    error_log("Erreur lors de la récupération des données de paiement: " . $e->getMessage());
    $_SESSION['error_message'] = "Une erreur est survenue lors de la récupération des données de paiement.";
}
        ...
        try {
            ...
            if (!$solution_to_pay) {
                ...
                if ($result) {
                    ...
                    $stmt = $pdo->prepare("
                        UPDATE solutions
                        SET payment_id = ?, payment_date = CURRENT_TIMESTAMP
                        WHERE id = ?
                    ");
                    ...
                    if ($result) {
                        ...
                        if ($result) {
                            ...
                            $stmt = $pdo->prepare("
                                ...
                                VALUES (?, 'payment_received', ?, ?, CURRENT_TIMESTAMP)
                            ");
...
$price = floatval($solution['price']);
echo number_format($price, 2);

        echo number_format($user_balance, 2); ?> €</div>
            ...
            <?php foreach ($pending_payments as $payment): ?>
                <div class="payment-card">
                    <div class="payment-card-header">
                        ...
                        <div class="payment-card-price"><?php echo number_format($payment['price'], 2); ?> €</div>
                    </div>
            ...
            <?php foreach ($payment_history as $history): ?>
                <div class="payment-card">
                    <div class="payment-card-header">
                        ...
                        <div class="payment-card-price"><?php echo number_format($history['amount'], 2); ?> €</div>
                    </div>
    ...
    <div class="payment-section" id="specific-section">
        <div class="payment-card">
            <div class="payment-card-header">
                ...
                <div class="payment-card-price"><?php echo number_format($solution['price'], 2); ?> €</div>
            </div>
        ...
        <div class="payment-form">
            <form method="POST" action="" id="payment-form">
                ...
                <button type="submit" class="btn-pay" id="submit-payment">
                    <i class="fas fa-lock"></i> Payer <?php echo number_format($solution['price'], 2); ?> €
                </button>
            </form>
        </div>
    </div>
