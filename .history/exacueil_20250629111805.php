...
// Récupérer les publications avec requêtes SQL Server
try {
    // Requête SQL Server corrigée
    $sql = "
        SELECT p.*, u.username, u.avatar_url,
               ...
               ISNULL((SELECT COUNT(*) FROM solutions WHERE problem_id = p.problem_id), 0) AS solution_count
        FROM problems p 
...
<div class="publications-grid">
    <?php if (empty($publications)): ?>
        <div style="text-align: center; padding: 40px; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            <i class="fas fa-code" style="font-size: 48px; color: #ddd; margin-bottom: 20px;"></i>
            <h3 style="color: #666;">Aucun problème disponible</h3>
            <p style="color: #999;">Soyez le premier à publier un problème!</p>
            <a href="expublier.php" class="btn btn-primary" style="margin-top: 15px;">
    ...
    <?php else: ?>
        ...
        <div class="publication-card">
            ...
            <div>
                <div class="status-dot <?= ($pub['has_solution'] ?? 0) > 0 ? 'solved' : 'unsolved'; ?>"></div>
...
$additional_scripts = "
    // Gestion des favoris - VERSION SQL SERVER
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchContainer.innerHTML = `
            <input type=\"text\" id=\"searchInput\" placeholder=\"Rechercher des problèmes...\" 
                   style=\"width: 300px; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 16px;\">
        `;
    ...
    console.log('✅ Page d\\'accueil chargée avec', document.querySelectorAll('.publication-card').length, 'problèmes');
";
