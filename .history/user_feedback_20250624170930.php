...
// Fonction d'échappement CSV standard
const escapeCSV = (text) =>
  ""${String(text).replace(/"/g, '""')}"";

const csvContent = [
  headers.map(escapeCSV).join(','), // sécuriser aussi les headers
  ...solutions.map(sol => [
    escapeCSV(sol.title),
    escapeCSV(sol.author),
    escapeCSV(sol.date),
    escapeCSV(sol.status),
    escapeCSV(sol.price),
    escapeCSV(sol.code),
    escapeCSV(sol.explanation)
  ].join(','))
].join('\n');
