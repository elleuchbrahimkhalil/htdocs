...
try {
    ...
    // Ajoutez ce code de débogage avant la requête
    error_log("Recherche des solutions pour l'utilisateur ID: " . $user_id);

    // Modifiez la requête pour être plus flexible avec les noms de colonnes
    $stmt = $pdo->prepare("
        SELECT s.*, p.title as problem_title, u.username, 
               COALESCE(u.name, u.username) as solver_name,
    ...
    // Ajoutez ce code de débogage après la requête
    error_log("Nombre de solutions trouvées: " . count($received_solutions));
