...
try {
    ...
    if (isLoggedIn()) {
        ...
        $stmt = $conn->prepare("
            SELECT p.*, u.username, u.avatar_url, u.name as author_name,
                   CASE WHEN EXISTS(SELECT 1 FROM favorites WHERE problem_id = p.problem_id AND user_id = @user_id) THEN 1 ELSE 0 END AS is_favorite,
                   CASE WHEN EXISTS(SELECT 1 FROM solutions WHERE problem_id = p.problem_id AND user_id = @user_id AND status IN ('approved','accepted')) THEN 1 ELSE 0 END AS is_solved
            FROM problems p 
    ...
    $solutions_stmt = $conn->prepare("
        ...
        WHERE s.problem_id = @problem_id AND s.status = 'approved'
        ORDER BY s.created_at DESC
    ");
    $solutions_stmt->bindValue('@problem_id', $problem_id, PDO::PARAM_INT);
    ...
    if (isLoggedIn()) {
        ...
        ");
        $user_solution_stmt->bindValue('@problem_id', $problem_id, PDO::PARAM_INT);
        $user_solution_stmt->bindValue('@user_id', $user_id, PDO::PARAM_INT);
        $user_solution_stmt->execute();
