...
try {
    ...
    // Requête de base pour récupérer le problème
    $sql = "
        SELECT 
            p.problem_id, 
            p.title, 
    ...
    // Ajouter les informations supplémentaires pour les utilisateurs connectés
    if (isLoggedIn()) {
        // Vérifier si l'utilisateur a résolu le problème
        $solved_stmt = $pdo->prepare("
            SELECT COUNT(*) 
            FROM solutions 
            ...
            AND status = 'accepted'
        ");
        $solved_stmt->bindValue(':problem_id', $problem_id, PDO::PARAM_INT);
        $solved_stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
        $solved_stmt->execute();
        $problem['is_solved_by_me'] = (int)$solved_stmt->fetchColumn();
        
        // Vérifier si le problème est en favori
        $fav_stmt = $pdo->prepare("
            ...
            AND user_id = :user_id
        ");
        $fav_stmt->bindValue(':problem_id', $problem_id, PDO::PARAM_INT);
        $fav_stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
        $fav_stmt->execute();
        $problem['is_favorite'] = (int)$fav_stmt->fetchColumn();
        
        // Vérifier si l'utilisateur a déjà soumis une solution
        $solution_stmt = $pdo->prepare("
            ...
            LIMIT 1
        ");
        $solution_stmt->bindValue(':problem_id', $problem_id, PDO::PARAM_INT);
        $solution_stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
        $solution_stmt->execute();
        $user_solution = $solution_stmt->fetch(PDO::FETCH_ASSOC);
    } else {
        $problem['is_solved_by_me'] = 0;
        $problem['is_favorite'] = 0;
        $user_solution = null;
    }
    ...
    $stats_stmt = $pdo->prepare("
        ...
        WHERE problem_id = :problem_id
    ");
    $stats_stmt->bindValue(':problem_id', $problem_id, PDO::PARAM_INT);
    $stats_stmt->execute();
    $solutions_stats = $stats_stmt->fetch(PDO::FETCH_ASSOC);
    
} catch (PDOException $e) {
    // Journalisation détaillée
    error_log("ERREUR PDO DÉTAILLÉE dans problem.php:");
    error_log("Message: " . $e->getMessage());
    error_log("Fichier: " . $e->getFile());
    error_log("Ligne: " . $e->getLine());
    error_log("Code erreur: " . $e->getCode());
    
    $_SESSION['error_message'] = "Erreur technique. Notre équipe a été notifiée.";
    header('Location: exacueil.php');
    exit;
} catch (Exception $e) {
    error_log("Erreur générale dans problem.php: " . $e->getMessage());
        ...
        <div class="problem-container">
            <div class="problem-header">
                ...
                <div class="problem-meta">
                    <span>Publié par <?php echo htmlspecialchars($problem['author_name'] ?? $problem['username']); ?></span>
            ...
            <div class="actions">
                <?php if (isLoggedIn()): ?>
                    <?php if ($problem['user_id'] == $_SESSION['user_id']): ?>
                        ...
                        <a href="edit_problem.php?id=<?php echo $problem['problem_id']; ?>" class="btn btn-outline">
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($user_solution): ?>
                            ...
                            <?php if ($user_solution['status'] === 'pending'): ?>
                                ...
                                <a href="submit_solution.php?problem_id=<?php echo $problem['problem_id']; ?>" class="btn btn-outline">
                            ...
                            <?php elseif ($user_solution['status'] === 'rejected'): ?>
                                ...
                                <a href="submit_solution.php?problem_id=<?php echo $problem['problem_id']; ?>" class="btn btn-primary">
