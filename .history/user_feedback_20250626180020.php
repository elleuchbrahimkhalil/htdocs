...
// Traitement des actions POST AVANT la récupération des données
    ...
    try {
        ...
        if (isset($_POST['accept_solution'])) {
            ...
            if ($solution_id > 0) {
                ...
                $debug_info[] = "🔍 Vérification des contraintes sur la colonne status";
                ...
                try {
                    ...
                    $constraint_query = "
                        ...
                        WHERE t.name = 'solutions' AND c.name = 'status'
                    ";
                    
                    $stmt = $pdo->prepare($constraint_query);
                    $stmt->execute();
                    $constraint = $stmt->fetch(PDO::FETCH_ASSOC);
                    
                    if ($constraint) {
                        $debug_info[] = "📋 Contrainte trouvée: " . $constraint['constraint_name'];
                        $debug_info[] = "📝 Définition: " . $constraint['definition'];
                    }
                    
                    // Vérifier les valeurs existantes
                    $stmt = $pdo->prepare("SELECT DISTINCT status FROM solutions");
                    $stmt->execute();
                    $existing_statuses = $stmt->fetchAll(PDO::FETCH_COLUMN);
                    $debug_info[] = "📊 Statuts existants: " . implode(', ', $existing_statuses);
                    
                } catch (Exception $e) {
                    $debug_info[] = "⚠️ Impossible de vérifier les contraintes: " . $e->getMessage();
                }
                ...
                if ($solution) {
                    $debug_info[] = "✅ Solution trouvée: ID " . $solution_id;
                    $debug_info[] = "📊 Statut actuel: " . $solution['status'];
                    
                    if ($solution['status'] === 'pending') {
                        // Essayer différentes valeurs pour le statut selon la contrainte
                        $new_status_options = ['approved', 'accepted', 'approve', 'accept'];
                        $update_success = false;
                        
                        foreach ($new_status_options as $new_status) {
                            try {
                                $debug_info[] = "🔄 Tentative avec statut: '$new_status'";
                                ...
                                $result = $stmt->execute([$new_status, $solution_id]);
                                
                                if ($result) {
                                    $debug_info[] = "✅ Mise à jour réussie avec statut: '$new_status'";
                                    ...
                                    $debug_info[] = "✅ Points mis à jour pour l'utilisateur " . $solution['user_id'];
                                    ...
                                    $_SESSION['debug_info'] = $debug_info; // Sauvegarder les infos de debug
                                ...
                                $debug_info[] = "❌ Échec avec '$new_status': " . $e->getMessage();
                                continue; // Essayer le statut suivant
                            }
                        }
                        
                        if (!$update_success) {
                            $error_message = "Impossible de mettre à jour le statut de la solution. Vérifiez les contraintes de la base de données.";
                            $debug_info[] = "❌ Toutes les tentatives de mise à jour ont échoué";
                        }
                        
                    } else {
                        $error_message = "Cette solution a déjà été évaluée (statut: " . $solution['status'] . ").";
                        $debug_info[] = "⚠️ Solution déjà évaluée";
                    }
                } else {
                    $error_message = "Solution introuvable ou vous n'êtes pas autorisé à l'accepter.";
                    $debug_info[] = "❌ Solution non trouvée ou accès refusé";
                }
            } else {
        ...
        if (isset($_POST['reject_solution'])) {
            ...
            if ($solution_id > 0) {
                ...
                if ($solution) {
                    if ($solution['status'] === 'pending') {
                        // Essayer différentes valeurs pour le rejet
                        $reject_status_options = ['rejected', 'reject', 'declined', 'refuse'];
                        $update_success = false;
                        
                        foreach ($reject_status_options as $reject_status) {
                            try {
                                $debug_info[] = "🔄 Tentative de rejet avec statut: '$reject_status'";
                                ...
                                $result = $stmt->execute([$reject_status, $solution_id]);
                                
                                if ($result) {
                                    $success_message = "La solution a été rejetée avec succès.";
                                    $debug_info[] = "✅ Rejet réussi avec statut: '$reject_status'";
                                    $update_success = true;
                                ...
                                $debug_info[] = "❌ Échec du rejet avec '$reject_status': " . $e->getMessage();
                                continue; // Essayer le statut suivant
                            }
                        }
                        
                        if (!$update_success) {
                            $error_message = "Impossible de rejeter la solution. Vérifiez les contraintes de la base de données.";
                            $debug_info[] = "❌ Toutes les tentatives de rejet ont échoué";
                        }
...
// Récupération des données SEULEMENT si on n'a pas redirigé
try {
    ...
    if ($user_problems_count == 0) {
        ...
        $debug_info[] = "🔍 Clause WHERE: " . ($where_filter ?: "Aucun filtre supplémentaire");
        ...
        $debug_info[] = "📊 Requête de comptage: $count_sql";
        ...
        if ($total_count > 0) {
            // Essayer d'abord avec une CTE (pour SQL Server)
            try {
                $cte_sql = "
                    ...
                    SELECT * FROM SolutionsCTE
                    WHERE RowNum > ? AND RowNum <= ?";
                
                $cte_params = array_merge($params, [$offset, $offset + $per_page]);
                $debug_info[] = "🔄 Tentative avec CTE SQL Server";
                
                $stmt = $pdo->prepare($cte_sql);
                $stmt->execute($cte_params);
                $received_solutions = $stmt->fetchAll(PDO::FETCH_ASSOC);
                
                $debug_info[] = "✅ CTE réussie, solutions récupérées: " . count($received_solutions);
                
            } catch (Exception $cte_error) {
                $debug_info[] = "❌ Erreur CTE: " . $cte_error->getMessage();
                $debug_info[] = "🔄 Basculement vers méthode alternative";
                
                // Méthode alternative : récupérer toutes les solutions et paginer avec PHP
                $fallback_sql = "
                    SELECT 
                        s.id,
                        s.problem_id,
...
// Récupérer les infos de debug de la session si elles existent
if (isset($_SESSION['debug_info'])) {
    $debug_info = array_merge($_SESSION['debug_info'], $debug_info);
