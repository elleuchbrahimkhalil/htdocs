...
if (!isset($_GET['id'])) {  // Correction: ajout de la parenthèse fermante
    header('Location: exacueil.php');  // Correction: redirection vers exacueil.php
    exit;
}

$conn = connect();
$problem_id = (int)$_GET['id'];
$user_id = isLoggedIn() ? $_SESSION['user_id'] : 0;  // Correction: vérification si l'utilisateur est connecté
    ...
    <div class="problem-container">
        ...
        <div class="problem-content">
            ...
            <?php if(!empty($problem['code'])): ?>
                <h3>Code</h3>
                <pre><?= htmlspecialchars($problem['code']) ?></pre>  <!-- Correction: suppression de la parenthèse supplémentaire -->
            <?php endif; ?>
            ...
            <?php if(count($solutions) > 0): ?>
                <?php foreach($solutions as $solution): ?>
                    <div class="solution">
                        ...
                        <pre><?= htmlspecialchars($solution['solution_code']) ?></pre>  <!-- Correction: suppression de la parenthèse supplémentaire -->
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
