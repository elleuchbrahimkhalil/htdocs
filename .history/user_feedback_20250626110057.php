...
try {
    ...
    if ($user_problems_count == 0) {
        ...
        if ($total_count > 0) {
            // CORRECTION: Utiliser TOP avec sous-requête au lieu de OFFSET/FETCH avec paramètres
            // Méthode 1: Utiliser ROW_NUMBER() avec CTE (Common Table Expression)
            $main_query = "
            ...
            if (empty($received_solutions) && $total_count > 0) {
                $debug_info[] = "⚠️ CTE n'a pas fonctionné, essai avec sous-requête";
                
                // Méthode 2: Récupérer tous les résultats et utiliser array_slice (pour petites données)
                if ($total_count <= 1000) { // Limite raisonnable
                    ...
                    // Appliquer la pagination côté PHP
                    $received_solutions = array_slice($all_solutions, $offset, $per_page);
                    
                    $debug_info[] = "✅ Solutions récupérées (méthode alternative): " . count($received_solutions);
                }
            }
        }
    }
    ...
    $debug_info[] = "📍 État SQL: " . $e->errorInfo[0] ?? 'N/A';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error_message) && isset($pdo)) {
    try {
...
$additional_scripts = "
    ...
    function enableAutoRefresh(intervalMinutes = 5) {
        ...
        setTimeout(() => {
            if (confirm('Actualiser la page pour voir les nouvelles solutions?')) {
                window.location.reload();
            } else {
