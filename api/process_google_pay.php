...
try {
    ...
    $paymentMethodType = $input['paymentData']['paymentMethodData']['type'];

    // Journaliser les informations de paiement (pour le débogage)
    error_log("Type de méthode de paiement: " . $paymentMethodType);
    error_log("Token reçu: " . substr($paymentToken, 0, 20) . "...");

    // Traiter le paiement selon la méthode
    switch ($paymentMethodType) {
        case 'CARD':
            // Traitement des paiements par carte via votre passerelle
            // Exemple avec Stripe (nécessite l'installation de la bibliothèque Stripe)
            /*
            ...
            */
            ...
            // Pour le moment, nous simulons un paiement réussi
        ...
        case 'BANK_TRANSFER':
            ...
            // Peut nécessiter une vérification manuelle
            $payment_success = true;
            break;
            
        default:
            // Méthode de paiement non prise en charge
            $pdo->rollBack();
            http_response_code(400);
            echo json_encode(['error' => 'Méthode de paiement non prise en charge']);
            exit;
    }
